...
// Goal: Update answers in a post
app.patch('/posts/:id', async (req, res, next) => {

  const { id } = req.params

  let postUpdated = await Post.findByIdAndUpdate(id, req.body, { new: true })
  res.send(postUpdated)
})

app.get('/posts/:id/answers', async (req, res, next) => {
...
app.post('/posts/:id/answers', async (req, res, next) => {

  // expect an answer item to be in req.body
  const {id} = req.params
  const post = await Post.findById(id)

  // add something to an embedded array
  post.answers.push(req.body)
  let postUpdated = await post.save()

  res.send(postUpdated.answers)
})


// UPDATING a subdocument (=embedded document)
app.patch('/posts/:postId/answers/:answerId', async (req, res, next) => {

  const { postId, answerId } = req.params
  const post = await Post.findById(postId)

  // grab subdocument
  let answerToUpdate = post.answers.id(answerId)

  // update the subdocument
  Object.assign(answerToUpdate, req.body)
...
app.delete('/posts/:postId/answers/:answerId', async (req, res, next) => {

  const { postId, answerId } = req.params
  const post = await Post.findById(postId)

  // grab subdocument
  let answerToDelete = post.answers.id(answerId)

  answerToDelete.remove() // removes item from array (but not directly updates DB!!!)

  let postUpdated = await post.save() // that here updates anwers array in DB!
